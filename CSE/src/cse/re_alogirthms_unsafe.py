...
def blur_image_from_attribution(image, attribution_map):
    ...
    #avg = np.array([-0.485, -0.456, -0.406], dtype=np.float32).reshape(3,1,1) 
    #avg = np.array([0.485, 0.456, 0.406], dtype=np.float32).reshape(3,1,1) 
    black_pixels = [-2.1179039301310043, -2.0357142857142856, -1.8044444444444445]
    
    avg = np.array(black_pixels, dtype=np.float32).reshape(3,1,1)
    #avg = np.float32(-0.4242)
    # avg should be [-0.485, -0.456, -0.406] to turn pixels black due to imagenet normalization
    # avg_img = np.where(average_img > 9999, average_img, avg)

    # attribution_map = attribution_map.detach().squeeze().cpu().numpy()
    # We might need to change the mask
    mask = [attribution_map]*3
    mask = np.array(mask)

    out = np.where(mask == np.array([0]*3).reshape(3,1,1), image.squeeze().cpu().numpy(), avg)
...
def region_explainability(image, segment_mask: np.array, top_n_start: int, model: torch.nn.Module,
    ...
    top_attrs, normalized_scores_list, _ = attribution_ranker(cam_processor_for_segments_output=avg_attr_scores,
                                                           num_top_attr=seg[2])
    ...
    while True:
        ...
        image_tensor_batch = torch.zeros(batch_size, 1, 3, 224, 224).to(device)
        ...
        for num in range(batch_size):
            total_attribution.append(np.zeros((224, 224)))
        ...
        np_output = model(image_tensor_batch.squeeze(1)).cpu().detach().numpy()
